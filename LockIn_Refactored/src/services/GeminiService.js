...
const { queueTextToSpeech } = require("./TtsService");
...
class GeminiService {
    ...
    try {
      ...
      this.session = await this.client.live.connect({
        ...
        callbacks: {
          ...
          onmessage: (message) => {
            // Only pass the 'text' value to textToSpeechInputStreaming if it exists
              ...
              functionDeclarations: [
                {
                  ...
                  description:
                    "Toggles the drawing canvas mode on/off. Use this when the user wants to draw, sketch, or annotate on the screen.",
                  parameters: {
                    type: "object",
                    properties: {},
                    required: [],
                ...
                },
                {
                  name: "draw_shape",
                  description:
                    "Draws a simple shape on the canvas. Use this when the user asks to draw geometric shapes like rectangles, circles, or lines.",
                  parameters: {
                    type: "object",
                    properties: {
                      shape: {
                      ...
                      x: {
                        type: "number",
                        description:
                          "X coordinate for the shape (0-100, percentage of screen width)",
                      },
                      y: {
                        type: "number",
                        description:
                          "Y coordinate for the shape (0-100, percentage of screen height)",
                      },
                      width: {
                        type: "number",
                        description:
                          "Width of the shape (0-100, percentage of screen width). For circles, this is the diameter.",
                      },
                      height: {
                        type: "number",
                        description:
                          "Height of the shape (0-100, percentage of screen height). Not used for circles.",
                      },
                      color: {
                        type: "string",
                        enum: [
                ...
                },
                {
                  name: "draw_stroke",
                  description:
                    "Draws a freehand stroke or mark at specific pixel coordinates. Use this when the user asks to draw at exact pixel positions or make marks/strokes.",
                  parameters: {
                    type: "object",
                    properties: {
                      x: {
                ...
                },
                {
                  name: "draw_parabola",
                  description:
                    "Draws a parabola curve for mathematical explanations. Perfect for teaching quadratic functions, physics trajectories, or any parabolic concepts.",
                  parameters: {
                    type: "object",
                    properties: {
                      centerX: {
                        type: "number",
                        description:
                          "X coordinate of the parabola center (0-100, percentage of screen width)",
                        default: 50,
                      },
                      centerY: {
                        type: "number",
                        description:
                          "Y coordinate of the parabola center (0-100, percentage of screen height)",
                        default: 50,
                      },
                      width: {
                        type: "number",
                        description:
                          "Width of the parabola (0-100, percentage of screen width)",
                        default: 30,
                      },
                      height: {
                        type: "number",
                        description:
                          "Height of the parabola (0-100, percentage of screen height)",
                        default: 20,
                      },
                      color: {
                        type: "string",
                      ...
                      direction: {
                        ...
                        enum: ["up", "down"],
                        description:
                          "Direction the parabola opens (up for U-shape, down for inverted U)",
                        default: "up",
                      },
                    },
                    required: [],
                ...
                },
                {
                  name: "robot_draw",
                  description:
                    "Uses robotjs to control the mouse and draw on the screen. Use this for drawing complex shapes or when drawing on the canvas is not working.",
                  parameters: {
                    type: "object",
                    properties: {
                      path: {
  ...
  handleMessage(message) {
    ...
    if (message.serverContent?.generationComplete) {
      ...
      if (this.currentTranscription && this.messageBuffer) {
        console.log("the messageBuffer", this.messageBuffer)
        console.log("the current Transcription", this.currentTranscription)
  ...
  async sendMessage(text) {
    ...
    try {
      console.log(
        "📤 Sending text message to Gemini:",
        text.substring(0, 50) + "..."
      );
  ...
  async sendAudio(audioData) {
    ...
    try {
      console.log(
        "🎤 Sending audio chunk to Gemini (size:",
  ...
  async sendImage(imageData) {
    ...
    try {
      console.log(
        "📸 Sending screenshot to Gemini (size:",
      ...
      switch (functionCall.name) {
        case "toggle_drawing_mode":
          ...
          if (global.mainWindow && !global.mainWindow.isDestroyed()) {
            try {
              const result = await global.mainWindow.webContents
                .executeJavaScript(`
          ...
          await this.session.sendRealtimeInput({
            functionResponse: {
              ...
              response: {
                success: true,
                message:
                  "Drawing mode has been toggled. You can now draw on the screen!",
              },
            },
          });
          break;
        ...
        case "draw_shape":
          ...
          if (global.mainWindow && !global.mainWindow.isDestroyed()) {
            try {
              ...
              const result = await global.mainWindow.webContents
                .executeJavaScript(`
                                console.log('🔷 Drawing shape with args:', ${JSON.stringify(
                                  args
                                )});
                                ...
                                if (window.canvasDrawing && typeof window.canvasDrawing.draw_shape === 'function') {
                                    ...
                                    const result = window.canvasDrawing.draw_shape(${JSON.stringify(
                                      args
                                    )});
        ...
        case "draw_stroke":
          ...
          if (global.mainWindow && !global.mainWindow.isDestroyed()) {
            try {
              ...
              const result = await global.mainWindow.webContents
                .executeJavaScript(`
                                console.log('✏️ Drawing stroke with args:', ${JSON.stringify(
                                  args
                                )});
                                ...
                                if (window.canvasDrawing && typeof window.canvasDrawing.draw_stroke === 'function') {
                                    ...
                                    const result = window.canvasDrawing.draw_stroke(${JSON.stringify(
                                      args
                                    )});
        ...
        case "draw_parabola":
          ...
          if (global.mainWindow && !global.mainWindow.isDestroyed()) {
            try {
              ...
              const result = await global.mainWindow.webContents
                .executeJavaScript(`
                                console.log('📈 Drawing parabola with args:', ${JSON.stringify(
                                  args
                                )});
                                ...
                                if (window.canvasDrawing && typeof window.canvasDrawing.draw_parabola === 'function') {
                                    ...
                                    const result = window.canvasDrawing.draw_parabola(${JSON.stringify(
                                      args
                                    )});
          ...
          await this.session.sendRealtimeInput({
            functionResponse: {
              ...
              response: {
                success: true,
                message: `Parabola has been drawn! Direction: ${
                  functionCall.args?.direction || "up"
                }, Color: ${functionCall.args?.color || "blue"}`,
              },
            },
          });
          break;
  ...
  async sendReconnectionContext() {
    ...
    try {
      const transcriptions = this.conversationHistory
        ...
        .filter(
          (transcription) => transcription && transcription.trim().length > 0
        );
